...
class Discriminator_Model():
    ...
    def predict_labels(self, x, audioFiles=None, threshold=0.5, raw_prob=False, batch_size=model_params['batch_size']):
        ...
        # print('===== y_pred below =====')
        # print(list(zip(audioFiles, y_pred)))
        # print('===== end y_pred =====')
        
        y_pred_list = [str(y[0]) for y in y_pred]
       
        if raw_prob:
            # return y_pred
            return dict(zip(audioFiles, y_pred_list))
        else:
            y_pred_labels = self.get_labels_from_prob(y_pred, threshold=threshold)
            return y_pred_labels
